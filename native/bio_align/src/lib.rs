...
// Copyright carried in compliance with Apache 2 license.
// NOTICE: this file has been modified to allow the exposure of the functions
// herein to be exposed using Rustler. Consequence changes to the types and
...
fn impl_hirschberg(
    ...
    // Invariant -- gap_val cannot be in either sequence
    if (seq_one.contains(&gap_val)) || (seq_two.contains(&gap_val)) {
        return Err( "Gap value {gap_val} cannot be present in either sequence");
    }
    ...
    let (aligned_seq_one_proc_pivot_l, aligned_seq_two_proc_pivot_l) = impl_hirschberg(
    ...
    let (mut aligned_seq_one_proc_pivot_r, mut aligned_seq_two_proc_pivot_r) = impl_hirschberg(
...
fn needleman_wunsch(
    ...
    return impl_needleman_wunsch(seq_one, seq_two, match_score, mismatch_score, indel_score, gap_val);
}
...
fn hirschberg(
    ...
    return impl_hirschberg(seq_one, seq_two, match_score, mismatch_score, indel_score, gap_val);
}

rustler::init!("Elixir.Bio.Align", [needleman_wunsch, hirschberg]);